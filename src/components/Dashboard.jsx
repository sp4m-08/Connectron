...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    onValue(tempRef, (snapshot) =>
      updateState(snapshot, setTemperature, "Temperature")
    );
    onValue(humiRef, (snapshot) =>
      updateState(snapshot, setHumidity, "Humidity")
    );
    onValue(flameRef, (snapshot) =>
      updateState(snapshot, setFlameDetected, "Flame Detected")
    );
    onValue(gasRef, (snapshot) =>
      updateState(snapshot, setGasLevel, "Gas Level")
    );
    onValue(tremorRef, (snapshot) =>
      updateState(snapshot, setTremorValue, "Tremor Value")
    );
    onValue(locationRef, (snapshot) =>
      updateState(snapshot, setLocationData, "Location Data")
    );
    onValue(impactRef, (snapshot) =>
      updateState(snapshot, setImpact, "Impact")
    );
  ...
  return (
    <div className="dashboard">
      ...
      <div className="data-boxes">
        ...
        <div className="data-box">
          <h3>Camera Feed</h3>
          <div className="camera-feed">📷 Live Feed</div>
        </div>
        <div className="data-box">
          <h3>Location</h3>
          <p>
        ...
        <div className="data-box">
          <h3>Flame</h3>
          <p>{flameDetected ? "✅ Safe" : "🔥 Detected"}</p>
        </div>
        <div className="data-box">
          <h3>Tremor</h3>
          <p>{tremorValue}</p>
      ...
      <div
        className={`notification-sidebar ${isNotificationOpen ? "open" : ""}`}
      >
        <h3
